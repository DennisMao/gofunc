...
func isBalanced(sub *Node) bool {
	balF := balFactor(sub)
	return balF > -2 && balF < 2
}

func balFactor(sub *Node) int {
	return getHeight(sub.left) - getHeight(sub.right)
}

// reBalanced can reBalanced an unbalanced node by 3-4 reconsitution way.
// Firstly,it will find the unbalanced type of the node (including zigzig,zagzag,zigzag,zagzig).
// And it calls 'rotate34At' with corresponding paramters.
func reBalanced(g *Node) {
	/*
		1.judege whether 'g' is balanced in recursion
		2.judege types zigzig or zigzag  respectively
		3.call ratate34At to rebalanced the node
	*/
}

// rotate34At can adjust nodes and rebalance by 3-4 reconsitution.
func rotate34At(a, b, c, t1, t2, t3, t4 *Node) {
...
func (tree *AvlTree) Insert(it string) *Node {
	if tree.Root == nil {
...
func (tree *AvlTree) Search(it string) *Node {
	if tree.Root == nil {
		return nil
	}
...
func (tree *AvlTree) Delete(it string) error {
...
//（2）前序遍历左子树
//（3）前序遍历右子树
func (tree *AvlTree) PreorderTraversal(subNode *Node, it ItemIterator) {
	if subNode == nil {
		return
	}
...
//（2）访问根结点
//（3）中序遍历右子树
func (tree *AvlTree) InorderTraversal(subNode *Node, it ItemIterator) {
	if subNode == nil {
		return
	}
...
//（2）后序遍历右子树
//（3）访问根结点
func (tree *AvlTree) PostorderTraversal(subNode *Node, it ItemIterator) {
	if subNode == nil {
		return
	}
...
// (2)出栈,输出
// (3)左孩子入栈、右孩子入栈
func (tree *AvlTree) LevelTraversal(subNode *Node, it ItemIterator) {
	if subNode == nil {
		return
	}
